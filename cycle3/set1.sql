CREATE TABLE employ(ssn INT PRIMARY KEY,ename VARCHAR(25) NOT NULL,design VARCHAR(25),dno INT,doj  DATE ,salary INT);
...
CREATE TABLE  Work_in(essn INT,pno INT,hours INT,FOREIGN KEY (essn) REFERENCES employ(ssn),FOREIGN KEY (pno) REFERENCES Proj(pnumber),
...
insert into employ (ssn,ename,design,dno,doj,salary)values (1,'Abhi','HR',2,'2009-07-12',70000),(2,'Bhama','Admin',1,'2008-03-10',75000),
...
select * from Work_in;
SELECT e.ename FROM employ e LEFT OUTER JOIN department d on d.dnumber=e.dno WHERE e.salary BETWEEN 30000 AND 40000 AND d.dnumber=5;
SELECT e.ename,d.dname FROM employ e LEFT OUTER JOIN department d on e.dno=d.dnumber ORDER BY d.dname ASC,e.ename ASC; 
SELECT p.pnumber,p.pname,count(e.ssn) FROM Work_in w LEFT OUTER JOIN Proj p on w.pno=p.pnumber LEFT OUTER JOIN employ e on w.essn=e.ssn GROUP BY p.pname,p.pnumber;
create VIEW emp_dept_view as select  * from employ NATURAL JOIN department;
CREATE ANY SYNONYM emp_dept for employe_dept_view;
select * from emp_dept;
Select * From employ e,department d WHERE e.dno=d.dnumber;
SELECT * FROM employ e,department d WHERE NOT(e.dno=d.dnumber);
SELECT * FROM employ e LEFT OUTER JOIN department d ON e.dno=d.dnumber; 
SELECT * FROM employ e RIGHT OUTER JOIN department d ON e.dno=d.dnumber; 
SELECT * FROM employ e INNER JOIN department d ON e.dno=d.dnumber;